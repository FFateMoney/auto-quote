...
from backend.common.models import StandardChunk
...
class StandardResolver:
    ...
    def resolve_for_row(
        ...
        *,
        ...
        usable_candidates = [c for c in candidates if self._is_seed_candidate(c.chunk)]
        if not usable_candidates:
            return self._missing(row, "未命中三级标题片段，按本地无标准处理")
        best_candidate = usable_candidates[0]
        return self._resolve_candidate(row, best_candidate, target_fields=target_fields or [], run_dir=run_dir)
