...
function findLatestStageWithRows(
  stages: FormStageSnapshot[],
  predicate: (row: FormRow) => boolean,
): FormStageSnapshot | undefined {
  for (let index = stages.length - 1; index >= 0; index -= 1) {
    const stage = stages[index];
    if (stage.items.some(predicate)) {
      return stage;
    }
  }
  return undefined;
...
function RejectedEquipmentPanel({
  ...
  const displayStage = useMemo(() => {
    if (activeStage?.items.some((row) => row.rejected_equipment.length > 0)) {
      return activeStage;
    }
    return findLatestStageWithRows(runState.form_stages, (row) => row.rejected_equipment.length > 0);
  }, [activeStage, runState.form_stages]);
  const rowsWithRejections = useMemo(
    () => (displayStage?.items ?? []).filter((row) => row.rejected_equipment.length > 0),
    [displayStage],
  );

  if (!displayStage || rowsWithRejections.length === 0) {
    return null;
  }
  ...
  return (
    <section className="panel">
      <div className="panel-header">
        <h3>被筛除设备表</h3>
        <p>
          逐台展示设备为什么被筛掉；当当前阶段没有这部分数据时，会自动回退显示最近一次有筛除记录的阶段。
          当前展示阶段：{displayStage.label}
        </p>
      </div>
...
function MatchedEquipmentPanel({
  ...
  const displayStage = useMemo(() => {
    if (activeStage?.items.some((row) => row.candidate_equipment_profiles.length > 0)) {
      return activeStage;
    }
    return findLatestStageWithRows(runState.form_stages, (row) => row.candidate_equipment_profiles.length > 0);
  }, [activeStage, runState.form_stages]);
  const rowsWithCandidates = useMemo(
    () => (displayStage?.items ?? []).filter((row) => row.candidate_equipment_profiles.length > 0),
    [displayStage],
  );

  if (!displayStage || rowsWithCandidates.length === 0) {
    return null;
  }
    ...
    <div className="app-shell">
      ...
      {runState ? (
        <>
          ...
          <MatchedEquipmentPanel activeStage={activeStage} runState={runState} />
